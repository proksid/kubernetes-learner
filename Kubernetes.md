...
<img src="attachment/c688298d51563632f4371307b8e8184e.png" />
...
5. Authentication gatekeeper 
	...
	Endpoint examples
   <img src="attachment/f0424901834f4edb518a6ddccb9da524.png" width="700" />
...
9. API [available/enabled/disabled admission controllers](#13.3.3.%20Operations)
...
### 4. ETCD
1. Distributed strongly consistent [key-value database](../../System%20Design/Database/DB%20Types%20and%20Data%20Model.md) for K8s cluster states
...
7. etcd is also used to store configuration details such as subnets, ConfigMaps, Secrets, etc
   ...
   <img src="attachment/1306de4d44b453be3231a80d1464ee8e.png" width="800" />
   <img src="attachment/5fd01d5436bd221259e38976b87d3d72.png" width="800" />
...
4. Serve (without *API*) the special **Static Pod**s
	...
	<img src="attachment/2c798bf5db2ae3d2e65b4c653a858edb.png" width="800" />
...
4. CRI examples:
	1. **containerd** plugin (aka cri-containerd)
	   ...
	   <img src="attachment/2dbb2fae11d5eb931164607e20e37d7f.png" width="800" />
	2. **cri-dockerd**
	   
	   <img src="attachment/1c7f33f9535f7188a46f4a6c71e1987e.png" width="800" />
### 4. Addons
1. DNS
2. Dashboard
3. Monitoring
...
2. Pod-to-Pod communicate via [CNI](#9.1.%20Container%20Network%20Interfaces%20/%20CNI)
3. External-to-Pod  
...
1. One of the nodes crashed
	...
	3. *ReplicaSet controller* in *kube-controller-manager* watches the change in actual state (via API Server), sees *.status.readyReplicas* < *.spec.replicas*
...
See [Admission control](#13.3.%20Admission%20control)

### 1.2. Scale space of names
### 1.3. Resource restriction
See [LimitRange](#2.7.3.%20LimitRange) and [ResourceQuota](#2.7.4.%20ResourceQuota)
...
1. Pod is a collection of one or more containers and volumes, where they share the same IP addresses
   <img src="attachment/ea46564177e4ed376cc6241ce6c8fce7.png" width="800" />
...
<img src="attachment/f98b0c8ede2ef4951da19a15e4338936.png" width="800" />
...
##### 2. readinessProbe
It probes if the container *is ready to respond to requests*, else the kubelet *removes its Pod's ip-address* from *Service Endpoints* - STOP routing TRAFFIC to the Pod. 
...
*LimitRange* - per Pod and per Container within the given **namespace**. Validations occur only at the Pod admission stage and do not impact the existing ones.
...
Provides limits across all Pods within the given **namespace** for cpu, memory, storage, external attributes and a number of entities
API server should run with `--enable-admission-plugins=` flag, which has *ResourceQuota*
When *quota* is defined for the given namespace, the resources for Pod/Container **must** be specified individually or with *LimitRange* defaults (an example when *LimitRange* mutation works before *ResourceQuota* validation).
...
8. [**CSI**](#^csi) drivers can provide different criteria  
...
6. Set/replace image on the fly t trigger rollout
...
2. *Recreate*  
   ...
   <img src="attachment/0a84fc170434990c7c75a1b353f474c3.png" width="800" />
...
`> kubectl set selector service my-service 'version=v2'`
   <img src="attachment/0dd146b14fc971cbbbd73288cba4ea59.png" width="800" />
#### 4.1.3. Canary
...
`> kubectl scale deployment primary replicas=0`
   <img src="attachment/f717711b4424c428ff10bc5a7b160f75.png" width="800" />
...
It usually works with a [Headless Service](#^headless-service) to give each Pod its own DNS name (e.g. db-0, db-1, …), which is critical for systems such as databases, queues, and clustered storage engines.
   ...
   <img src="attachment/a7bd0900a9eec7e8135d042c17b5ccc9.png" width="800" />
### Use cases
...
1. *DaemonSet* controller creates *spec.affinity.nodeAffinity* within the [Pod template](#2.3.2.%20**Affinity**%20(Node%20label,%20Pod%20affinity)) context.
2. *DaemonSet* controller adds a special set of tolerations within the [Pod template](#2.3.1.%20**Taints**%20(Node)%20/%20**Tolerations**%20(Pod)) context to run its Pod on Nodes that are marked unschedulable for application Pods.
3. Scheduler assigns *nodeName* within the [Pod template](#2.3.4.%20nodeName) context.
4. The default scheduler can be replaced with an assigned one with *.spec.schedulerName* within the Pod template context.
5. The user can restrict a group of Nodes with *spec.affinity.nodeAffinity* and *spec.nodeSelector* within the [Pod template](#2.3.3.%20nodeSelector) context.
...
2. Creates a Job with *.spec.jobTemplate* and Cron-like schedule *.spec.schedule*
   <img src="attachment/e351af382b44ab9b5222aabd1d22908e.png" width="800" />
3. Generate a skeleton manifest
 ...
 *ClusterIP* - provides a base cluster-wide *stable internal FQDN* to serve a set of Pods. Every pod has its unique cluster-wide ip-address, but they are ephemeral as well as their Pods.  All types inherit *ClusterIP*, excluding *ExternalName*.
 ...
 1. **Base Service** *.spec.ClusterIP* = "ip-address" - it provides FQDN and a stable internal ip-address that is routed within the cluster *service*.*namespace*.svc.cluster.local
    ...
    <img src="attachment/bfd8e3d30dca66463da742e4280c4ec5.png" width="800" />  
...
1. Ingress Controller processes and routes HTTP/HTTPS (Layer 7) requests according to rules to Services and Resources. This is an alternative to *NodePort* and LoadBalancer
   <img src="attachment/328dcebc69d461f19787db9ab560ec4c.png" width="800" />
...
2. Simple fanout  - ingress -> URI paths (*.spec.rules[\*].http.paths[\*].path*) -> service
   ...
   <img src="attachment/66c3645994ab443541c5176dc813d019.png" width="800" />
...
3. Name-based virtual hosting - ingress -> FQDN (*.spec.rules[\*].host*) -> services  
   ...
   <img src="attachment/0ef78b08c52ea91e9dce8ebc7f6f1812.png" width="800" />
4. TLS + [TLS Secret](#11.4.%20Secret%20TLS%20)
...
4. [The AND or OR logic of rules](https://kubernetes.io/docs/concepts/services-networking/network-policies/#behavior-of-to-and-from-selectors)
	...
	<img src="attachment/01ff374a4c2fadd94c6e16e8401a6bd5.png" width="800" />
   ...
   <img src="attachment/99034ccbd7d5200c08713a5dd35bf2c4.png" width="800" />
...
This *ConfigMap* can be mounted [as a volume](#10.3.%20Config%20file/directory) - each key will be a filename that comprises its value.

### 10.2. Env config file
`> kubectl create configmap test-config-map --from-env-file=test-config.properties`
And use [as env vars](#10.1.%20Literal%20config) or mount [as a volume](#10.3.%20Config%20file/directory) - each key will be a filename that comprises its value.
...
This *Secret* can be mounted [as a volume](#11.3.%20Secret%20file) - each key will be a filename that comprises its value.

### 11.2. Env secret file
 `> kubectl create secret generic test-secret --type=Opaque --from-env-file=secret.env`
 And use [as env vars](#11.1.%20Literal%20secret) or mount [as a volume](#11.3.%20Secret%20file) - each key will be a filename that comprises its value.
...
If it's required to enter values declaratively to .yaml, they should be encoded
...
3. To read/decode the token:  
   ...
   `> jq -R 'split(".") | select(length > 0) | .[0],.[1] | @base64d| fromjson' <<<  token-long-content`  
	   or 
...
4. Create *Role* and *RoleBinding* to bind resource permissions and SA (see [Authorization](#13.2.%20Authorization))
...
1. Webhook - by third-party decisions
...
4. Webhooks - sending a request to an external Admission Webhook Server
	...
	2. ValidatingAdmission
		   <img src="attachment/8abc334c205bf276ae69f043193466e1.png" width="800" />
...
2. In-Tree Volume drivers
	...
	4. *configMap* - (EV) ([usage](#10.3.%20Config%20file))
	5. *secret* - (EV) ([usage](#11.3.%20Secret%20file))
	6. downwardAPI - (EV), to inject Pod and Container fields into Container - metadata, spec, status, resource  
	   
	   
3. [Out-of-Tree Volume drivers](https://github.com/kubernetes-csi/external-provisioner) - Container Storage Interface (**CSI**)  ^csi
...
##### 1. Static: **PVC -> PV**
<img src="attachment/6d7d6dae104d1e3839d8884257cadc84.png" width="800" />  
   
   
##### 2. Dynamic: **PVC -> PV StorageClasses**
<img src="attachment/9572e1a7a476590f96a26995a2f8f75a.png" width="800" />


#### 15.3.2. Binding criteria.
<img src="attachment/8c9b15ce4086f8c04f21b725e87f18ce.png" width="800" />
...
```
<img src="attachment/296a7a54744847c558446059ffbd7d21.png" width="800" />
...
Defines how PersistentVolumes are [dynamically provisioned](#2.%20**Dynamic**%20PVC%20->%20PV%20StorageClasses) for each Pod in a [ StatefulSet](#5.%20StatefulSet). In other words, each Pod in the StatefulSet gets its own PersistentVolumeClaim based on this template. 


<img src="attachment/efbada14b6072726af5f870e1eb79e3c.png" width="800" /> 
...
3. Points of
	...
	3. API access
		1. [Authentication (webhook backend)](#13.1.%20Authentication%20authentication)
		2. [Authorization (webhook backend)](#13.2.%20Authorization%20authorization)
		3. [Dynamic Admission Control](#13.3.%20Admission%20control%20admission-control)
	4. Infrastructure
		1. [CSI](#^csi)
		2. [CNI](#9.1.%20Container%20Network%20Interfaces%20/%20CNI)
	5. Scheduling  
